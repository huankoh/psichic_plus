...
def replace_non_standard_residues(sequence, res_table):
    # Convert the residue table to a set for faster membership checking
    res_set = set(res_table)
    
    # Replace any character not in the residue table with 'X'
    cleaned_sequence = ''.join([res if res in res_set else 'X' for res in sequence])
...
def protein_init_for_mutation(keys, seqs, folder_path):
    assert len(seqs) == len(keys)
    ...
    for key, seq in tqdm(zip(keys, seqs), total=len(seqs),desc='Protein Initialization'):
        # Check if the key does NOT contain 'pos' or 'neg'
        if 'pos' not in key and 'neg' not in key:
            # Construct the file path
            file_path = os.path.join(folder_path, f"{key}.pt")
            
            # Check if the file already exists
            if os.path.exists(file_path):
                # If it exists, load it and continue to the next iteration
                result_dict[key] = torch.load(file_path)
                continue
            print('Creating protein init for: ', key, 'at ', file_path)
            ...
            # Add to the result_dict
            result_dict[key] = result
        
        else:
            # For keys containing 'pos' or 'neg', process without saving
            seq = replace_non_standard_residues(seq, pro_res_table)
            seq_feat = seq_feature(seq)
            token_repr, contact_map_proba, logits = esm_extract(model, batch_converter, seq, layer=33, approach='last', dim=1280)
