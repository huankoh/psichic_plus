...
def protein_degree_from_path(protein_path):
    protein_max_degree = -1 
    protein_files = os.listdir(protein_path)
    for pfile in protein_files:
        v = torch.load(os.path.join(protein_path, pfile))
        node_num = len(v['seq'])
...
def ligand_degree_from_dict(ligand_dict):
    ...
    for k, v in tqdm(ligand_dict.items()):
        ...
        mol_edge_index = v['atom_edge_index']
...
def store_result(df, attention_dict, ids, interaction_keys, 
                protein_dict, ligand_path, 
                 reg_pred=None, reg_alpha=None, reg_beta=None, 
                 result_path='', save_interpret=True, save_cluster=False):
    ...
    for idx, key in enumerate(interaction_keys):
        # Match row by ID, not by Protein/Ligand
        ...
        if save_interpret:
            for pair_id in df[matching_row]['ID']:
                ...
                ligand_path_out = os.path.join(pair_path,'ligand.pkl')
                successful_ligand = store_ligand_score(ligand_smiles, atom_types, 
...
def virtual_screening(screen_df, model, data_loader, result_path, protein_dict=None, ligand_path=None,  save_interpret=True, device='cpu', save_cluster=False):
    reg_preds = []
    reg_alphas = []
    reg_betas = []
    ids_all = []
    ...
    with torch.no_grad():
        for data in tqdm(data_loader):
            data = data.to(device)
            reg_alpha, reg_beta, ortho_loss, cluster_loss, attention_dict = model(data)
            reg_pred = reg_alpha / (reg_alpha + reg_beta) * 12
            reg_pred = reg_pred.squeeze().reshape(-1).cpu().numpy()
            reg_alpha = reg_alpha.squeeze().reshape(-1).cpu().numpy()
            reg_beta = reg_beta.squeeze().reshape(-1).cpu().numpy()
            reg_preds.append(reg_pred)
            reg_alphas.append(reg_alpha)
            reg_betas.append(reg_beta)
            interaction_keys = list(zip(data.prot_key, data.mol_key))
            # Collect ids for this batch
            batch_ids = data.id if isinstance(data.id, list) or isinstance(data.id, np.ndarray) else [data.id]
            ids_all.extend(batch_ids)
            screen_df = store_result(
                screen_df, attention_dict, batch_ids, interaction_keys, 
                protein_dict, ligand_path, 
                reg_pred, reg_alpha, reg_beta, 
